...
const AdminArticles = () => {
  ...
  // Fetch articles
  const { data: articlesData, isLoading, refetch } = useGetArticlesQuery({
  ...
  // Sort articles (client-side for additional sorting)
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteArticle(articleToDelete._id).unwrap();
      toast.success(`${articleToDelete.title} deleted successfully`);
  ...
  const handleToggleFeatured = async (article) => {
    try {
      await toggleFeatured(article._id).unwrap();
  ...
  // Handle toggle editor's pick
  const handleToggleEditorsPick = async (article) => {
    try {
      await toggleEditorsPick(article._id).unwrap();
      toast.success(article.isEditorsPick ? 'Removed from editor\'s pick' : 'Added to editor\'s pick');
      refetch();
    } catch (error) {
      toast.error(error?.data?.message || 'Failed to update editor\'s pick status');
    }
  };
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">Manage your blog articles and posts</p>
          </div>
          <button
            onClick={() => navigate('/admin/articles/new')}
            className="flex items-center gap-2 px-4 py-2 bg-[#1B3766] text-white rounded-lg font-medium hover:bg-[#142952] transition-all duration-300"
          >
            <FaPlus />
            Write New Article
          </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="w-10 h-10 bg-[#1B3766]/10 rounded-full flex items-center justify-center">
                <FaNewspaper className="text-[#1B3766]" />
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">
                  {articles.filter(a => a.isEditorsPick).length}
                </p>
              </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B3766] focus:border-transparent text-sm"
              />
              {searchTerm && (
                <button
            ...
            <div className="flex flex-wrap gap-2">
              <select
                ...
                className="flex-1 px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B3766] text-sm"
              >
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B3766] text-sm"
                >
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 text-center py-12">
            ...
            <p className="text-gray-500 mb-4">Try adjusting your search or create a new article</p>
            <button
              onClick={() => navigate('/admin/articles/new')}
              className="px-4 py-2 bg-[#1B3766] text-white rounded-lg hover:bg-[#142952] transition-colors"
            >
              Write New Article
            </button>
          </div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Editor's Pick</th>
                    ...
                    {paginatedArticles.map((article) => {
                      ...
                      return (
                        <tr key={article._id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className="text-sm text-gray-600">{article.views || 0}</span>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-center">
                            ...
                            >
                              {article.isFeatured ? <FaStar className="text-lg" /> : <FaRegStar className="text-lg" />}
                            </button>
                          </td>
                          <td className="px-6 py-4 text-center">
                            <button
                              onClick={() => handleToggleEditorsPick(article)}
                              className="text-red-400 hover:text-red-500 transition-colors"
                              title={article.isEditorsPick ? 'Remove from editor\'s pick' : 'Mark as editor\'s pick'}
                            >
                              {article.isEditorsPick ? <FaFire className="text-lg" /> : <FaRegStar className="text-lg" />}
                            </button>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                onClick={() => navigate(`/bizzed/articles/${article.slug}`)}
                                target="_blank"
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 mt-6">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 hover:border-[#1B3766] hover:text-[#1B3766] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  <FaChevronLeft />
                </button>
