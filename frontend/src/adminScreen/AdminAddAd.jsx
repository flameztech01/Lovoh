...
import { useCreateAdMutation } from '../slices/adsApiSlice';
...
const AdminAddAd = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    subtitle: duplicateFrom?.subtitle || '',
    description: duplicateFrom?.description || '',
    mediaType: duplicateFrom?.mediaType || 'image',
    ctaText: duplicateFrom?.ctaText || 'Learn More',
    ctaLink: duplicateFrom?.ctaLink || '',
    pages: duplicateFrom?.pages || ['uduua'],
    placements: duplicateFrom?.placements || ['hero'],
    bgColor: duplicateFrom?.bgColor || 'from-[#0043FC] to-[#0038D4]',
    accentColor: duplicateFrom?.accentColor || '#79FFFF',
    status: duplicateFrom?.status || 'active',
    priority: duplicateFrom?.priority || 1,
    startDate: duplicateFrom?.startDate ? new Date(duplicateFrom.startDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    endDate: duplicateFrom?.endDate ? new Date(duplicateFrom.endDate).toISOString().split('T')[0] : '',
    displayOrder: duplicateFrom?.displayOrder || 0,
  });

  // Media state
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(duplicateFrom?.image || '');
  const [videoFile, setVideoFile] = useState(null);
  const [videoPreview, setVideoPreview] = useState(duplicateFrom?.video || '');
  const [thumbnailFile, setThumbnailFile] = useState(null);
  const [thumbnailPreview, setThumbnailPreview] = useState(duplicateFrom?.thumbnail || '');
  ...
  const handleSubmit = async (e) => {
    ...
    // Debug log
    console.log('Submitting ad with:');
      ...
      <div className="px-4 py-6 md:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
            {duplicateFrom ? 'Duplicate Ad' : 'Create New Ad'}
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            {duplicateFrom ? 'Create a copy of an existing advertisement' : 'Create a new advertisement for your pages'}
          </p>
        </div>
            ...
            <div className="lg:w-96 space-y-5">
              ...
              <div className="hidden lg:block bg-gray-50 rounded-xl border border-gray-200 p-5">
                ...
                <ul className="space-y-2 text-sm text-gray-600">
                  ...
                  <li className="flex items-start gap-2">
                    <FaChartLine className="text-[#0043FC] text-xs mt-0.5" />
                    <span>Set end dates for time-sensitive promotions</span>
                  </li>
                </ul>
              </div>
            </div>
                ...
                >
                  {isLoading || uploading ? (
                    ...
                    <>
                      ...
                      {duplicateFrom ? 'Create Duplicate' : 'Create Ad'}
                    </>
                  )}
                </button>
              </div>
