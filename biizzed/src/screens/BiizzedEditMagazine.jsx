...
const BiizzedEditMagazine = () => {
  ...
  // Populate form when magazine loads
  useEffect(() => {
    if (magazine) {
      setFormData({
      ...
      setExistingCover(magazine.coverImage || '');
      setExistingPdfUrl(magazine.pdfUrl || '');
  ...
  const handleSubmit = async (e) => {
    ...
    // For published (non‑coming soon) we need a PDF (either existing or new)
    if (!formData.comingSoon && !existingPdfUrl && !pdfFile) {
    ...
    // Determine status: if comingSoon true -> 'coming_soon', else user can choose or default 'published'
    const newStatus = formData.comingSoon ? 'coming_soon' : (formData.status === 'published' ? 'published' : 'draft');
    fd.append('status', newStatus);

    if (coverImage) fd.append('coverImage', coverImage);
    if (pdfFile) fd.append('pdf', pdfFile);
    // We keep existing files if not replaced (backend handles via keep* flags? We'll just send new files when present)
        ...
        <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600 mt-1">
                  {formData.comingSoon
                    ...
                    : "Requires a PDF file. Magazine will be published (or draft if status is draft)."}
                </p>
              </div>
            </label>
          </div>
            ...
            {!coverPreview ? (
              ...
              <div className="relative max-w-[250px] mx-auto">
                ...
                <p className="text-[10px] text-gray-400 text-center mt-1">
                  {coverImage ? 'New image selected' : 'Current cover'}
                </p>
              </div>
            )}
          </div>
            ...
            {existingPdfUrl && !pdfFile && (
              <div className="mb-3 p-3 bg-gray-50 rounded-lg text-sm text-gray-600 flex items-center justify-between">
                <div className="flex items-center gap-2 truncate">
                  ...
                  <span className="truncate">Current PDF: {existingPdfUrl.split('/').pop()}</span>
                </div>
                <button
                  type="button"
                  onClick={() => setPdfFile(null)} // No direct removal, just allow replace
          ...
          {!formData.comingSoon && (
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-200">
              ...
              <p className="text-xs text-gray-400 mt-1">Published magazines will be visible to everyone and send notifications.</p>
            </div>
          )}
...
export default BiizzedEditMagazine;