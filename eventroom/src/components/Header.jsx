// components/Header.jsx – Clean cross‑domain header for LovohCreate
import React, { useEffect, useRef, useState, useCallback } from "react";
import { Link, useNavigate, useLocation } from "react-router-dom";

// --- Domain constants (adjust if your domains differ) ---
...
// Helper: get the correct URL for a main‑domain path (always absolute to MAIN_DOMAIN)
const getMainDomainUrl = (path) => `${MAIN_DOMAIN}${path}`;

// Helper: get URL for a sub‑brand (used for Biizzed, Uduua, EventRoom)
const getSubdomainUrl = (brand) => SUBDOMAINS[brand] || MAIN_DOMAIN;

// Decide if a link should be an absolute URL (external) or relative
const getLinkHref = (to) => {
  // Main domain pages – always absolute when not already on main domain
  const mainPages = ["/", "/about", "/work", "/services", "/contact"];
  if (mainPages.includes(to)) {
    // If we are on a subdomain, go to main domain
    if (currentSub !== null) return getMainDomainUrl(to);
    // On main domain, use relative routing
    return to;
  }
  // Sub‑brand links
...
const Header = () => {
  ...
  // Logo state (no crossfade for simplicity, but can be re‑added)
  ...
  const handleNavigation = useCallback(
    (to) => {
      ...
      // If it's an absolute URL (starts with http), do full page navigation
  ...
  // Desktop: click outside closes dropdown only
  useEffect(() => {
      ...
      <nav className="max-w-7xl mx-auto" ref={navContainerRef}>
        <div className="relative rounded-full bg-white/95 backdrop-blur-md border border-blue-100 shadow-[0_10px_35px_rgba(37,72,153,0.10)] px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo - always links to main domain home */}
            ...
            <div className="hidden md:flex items-center gap-2 lg:gap-3">
              ...
              {/* Sub‑brand links */}
              <button
                onClick={() => handleNavigation("/biizzed")}
                className="text-gray-700 hover:text-blue-700 px-4 py-2 text-sm font-medium rounded-full transition-all duration-200 hover:bg-blue-50"
              >
                Biizzed
              </button>
              <button
                onClick={() => handleNavigation("/uduua")}
                className="text-gray-700 hover:text-blue-700 px-4 py-2 text-sm font-medium rounded-full transition-all duration-200 hover:bg-blue-50"
              >
                Uduua
              </button>
