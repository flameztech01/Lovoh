// screens/EventDashboardEventRegistrations.jsx
...
const EventDashboardEventRegistrations = () => {
  ...
  const exportToCSV = () => {
    ...
    const formatCheckInTime = (reg, isAttendee = false, attendee = null) => {
      if (isAttendee && attendee) return attendee.checkedInAt ? formatDate(attendee.checkedInAt) : '';
    ...
    filteredRegistrations.forEach((reg) => {
      ...
      if (reg.customFormResponses) {
        reg.customFormResponses.forEach((resp) => {
          if (resp.label && buyerRow.hasOwnProperty(resp.label)) buyerRow[resp.label] = getCustomValue(resp.value);
        });
      }
      ...
      if (reg.additionalAttendees?.length) {
        reg.additionalAttendees.forEach((att) => {
          const guestRow = {};
          ...
          if (reg.customFormResponses) {
            reg.customFormResponses.forEach((resp) => {
              if (resp.label && guestRow.hasOwnProperty(resp.label)) guestRow[resp.label] = getCustomValue(resp.value);
            });
          }
  ...
  const bulkMail = () => {
    ...
    filteredRegistrations.forEach((reg) => {
      if (reg.email?.trim()) emailSet.add(reg.email.trim());
      reg.additionalAttendees?.forEach((att) => {
        if (att.email?.trim()) emailSet.add(att.email.trim());
      });
    });
  ...
  const handleManualCheckIn = async () => {
    ...
    try {
      await checkInAttendee(ticketId).unwrap();
      toast.success(`${selectedAttendee?.name || selectedRegistration?.name} checked in!`);
      closeCheckInModal();
      refetchRegistrations();
    } catch (error) {
    ...
    return (
      <EventDashboardSidebar>
        <div className="text-center py-20">
          ...
          <button onClick={() => navigate('/dashboard/events')} className="px-4 py-2 bg-[#1B3766] text-white rounded-lg">
            Back to My Events
          </button>
        </div>
      </EventDashboardSidebar>
      ...
      {showFilterPopup && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50" onClick={closeFilterPopup}>
          ...
          >
            ...
            <button onClick={() => handleFilterSelect('all')} className={`w-full text-left p-3 rounded-lg mb-2 ${statusFilter === 'all' ? 'bg-[#1B3766]/10 font-medium text-[#1B3766]' : 'hover:bg-gray-100 text-gray-700'}`}>All</button>
            <button onClick={() => handleFilterSelect('confirmed')} className={`w-full text-left p-3 rounded-lg mb-2 ${statusFilter === 'confirmed' ? 'bg-[#1B3766]/10 font-medium text-[#1B3766]' : 'hover:bg-gray-100 text-gray-700'}`}>Confirmed</button>
            <button onClick={() => handleFilterSelect('pending')} className={`w-full text-left p-3 rounded-lg ${statusFilter === 'pending' ? 'bg-[#1B3766]/10 font-medium text-[#1B3766]' : 'hover:bg-gray-100 text-gray-700'}`}>Pending</button>
            <button onClick={closeFilterPopup} className="w-full mt-4 py-2.5 border border-gray-200 rounded-lg text-gray-700 text-sm">Cancel</button>
          </div>
        </div>
      )}
    </>
  ...
  const ModalContainer = ({ show, onClose, anim, children }) => {
    if (!show) return null;
    return (
      <div className={`fixed inset-0 z-50 flex ${isMobile ? 'items-end' : 'items-center'} justify-center bg-black/50`} onClick={onClose}>
        <div
          className={`bg-white ${isMobile ? 'rounded-t-2xl w-full max-w-md p-4 max-h-[90vh] overflow-y-auto' : 'rounded-2xl max-w-md w-full p-6'} ${isMobile ? 'transform transition-transform duration-300 ease-out' : ''}`}
  ...
  const detailsContent = selectedRegistration && (
    <>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={closeDetailsModal} className="p-2 hover:bg-gray-100 rounded-lg"><FaTimes /></button>
      </div>
          ...
          <DetailSection icon={FaUserFriends} title="Additional Attendees">
            {selectedRegistration.additionalAttendees.map((att, idx) => (
              <div key={idx} className="bg-white rounded-lg p-2 border border-gray-100">
                ...
                <p className="text-[10px] text-gray-500">Ticket: {att.ticketId || '—'} • Seat: {att.seatNumber || '—'} • Email: {att.email || '—'}</p>
              </div>
            ))}
          </DetailSection>
        )}
          ...
          <DetailSection icon={FaLayerGroup} title="Additional Info">
            {selectedRegistration.customFormResponses.map((resp, idx) => (
              <div key={idx} className="mb-2">
                ...
                <p className="text-sm text-gray-900">{Array.isArray(resp.value) ? resp.value.join(', ') : resp.value || '—'}</p>
              </div>
            ))}
          </DetailSection>
        )}
      ...
      <div className="border-2 border-dashed border-[#1B3766] rounded-xl p-4 mb-4">
        ...
        {selectedRegistration.additionalAttendees?.length > 0 && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            ...
            {selectedRegistration.additionalAttendees.map((att, idx) => (
              <div key={idx} className="text-xs text-gray-600 mb-1"><span className="font-medium">{att.name || 'Guest'}</span> • Seat: {att.seatNumber} • {att.checkedIn ? '✅' : '⏳'}</div>
            ))}
          </div>
        )}
      </div>
      <button onClick={closeTicketModal} className="w-full py-2.5 border border-gray-200 rounded-lg text-gray-700 text-sm">Close</button>
    </div>
  );
  ...
  return (
    <EventDashboardSidebar>
      <div className="p-3 md:p-6 relative">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-3 mb-5">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search by name, email, ticket ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-9 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B3766] text-sm" />
                    ...
                    <div className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500 mt-1 flex-wrap">
                          <span>{reg.email}</span>{reg.phone && <span>📞 {reg.phone}</span>}<span>•</span><span>{reg.ticketType || 'General'}</span><span>•</span><span>Qty: {reg.quantity || 1}</span><span>•</span><span className="font-mono">{reg.ticketId}</span><span>•</span><span><FaChair className="inline text-[10px]" /> {reg.seatNumber}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {reg.ticketCheckedIn ? <span className="text-[10px] text-green-600 font-medium px-2">✅ In</span> : (reg.status === 'confirmed' && <button onClick={() => openCheckInModal(reg)} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs hover:bg-green-700">Check In</button>)}
                          ...
                          <div key={idx} className="px-4 py-2.5 flex items-center gap-4 border-b border-gray-100 last:border-0">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                                {att.email && <span>{att.email}</span>}{att.phone && <span>📞 {att.phone}</span>}<span className="font-mono">{att.ticketId}</span>{att.seatNumber && <span><FaChair className="inline text-[10px]" /> {att.seatNumber}</span>}
                              </div>
                            </div>
                            {att.checkedIn ? <span className="text-[10px] text-green-600 font-medium px-2">✅ In</span> : <button onClick={() => openCheckInModal(reg, att)} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs hover:bg-green-700">Check In</button>}
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-semibold text-gray-900 text-sm">{reg.name}</p>
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${status.color}`}><status.icon className="text-[10px]" /> {status.label}</span>
                          {hasAdditional && <span className="text-[10px] text-blue-600"><FaUserFriends className="inline mr-0.5" />+{reg.additionalAttendees.length}</span>}
                        </div>
                        <div className="text-xs text-gray-500 mt-1 space-x-1"><span>{reg.email}</span>{reg.phone && <span>📞 {reg.phone}</span>}</div>
                      ...
                      <div className="flex flex-col items-end gap-1 ml-2">
                        {reg.ticketCheckedIn ? <span className="text-[10px] text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-full">✅ In</span> : (reg.status === 'confirmed' && <button onClick={() => openCheckInModal(reg)} className="px-2 py-1 bg-green-600 text-white rounded text-[11px]">Check In</button>)}
                        ...
                        {reg.additionalAttendees.map((att, idx) => (
                          <div key={idx} className="flex items-center justify-between text-xs">
                            <div className="flex items-center gap-1.5">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center ${att.checkedIn ? 'bg-green-100' : 'bg-gray-100'}`}><FaUser className="text-[9px] text-gray-500" /></div>
                              <div><span className="font-medium text-gray-800">{att.name || 'Guest'}</span><div className="text-[10px] text-gray-400">{att.ticketId}</div></div>
                            </div>
                            {att.checkedIn ? <span className="text-green-600 text-[10px] bg-green-50 px-1.5 py-0.5 rounded-full">✅ In</span> : <button onClick={() => openCheckInModal(reg, att)} className="px-2 py-0.5 bg-green-600 text-white rounded text-[10px]">Check In</button>}
                          </div>
                        ))}
                      </div>
                    )}
        ...
        {/* Modals */}
        <ModalContainer show={showDetailsModal} onClose={closeDetailsModal} anim={detailsModalAnim}>{detailsContent}</ModalContainer>
        <ModalContainer show={showCheckInModal} onClose={closeCheckInModal} anim={checkInModalAnim}>{checkInContent}</ModalContainer>
        <ModalContainer show={showTicketModal} onClose={closeTicketModal} anim={ticketModalAnim}>{ticketContent}</ModalContainer>
        <ScannerModal />
      </div>
    </EventDashboardSidebar>
  );
...
const StatCardMobile = ({ label, value, color, textColor, icon: Icon, isCurrency }) => (
  <div className={`rounded-xl p-3 ${color}`}>
    <div className="flex items-center justify-between">
      <div><p className="text-[11px] text-gray-500">{label}</p><p className={`text-base font-bold mt-0.5 ${textColor} ${isCurrency ? 'text-xs' : ''}`}>{value}</p></div>
      <div className="w-8 h-8 rounded-full bg-white/60 flex items-center justify-center"><Icon className={`text-sm ${textColor}`} /></div>
    </div>
  </div>
);
...
const EmptyState = () => (
  <div className="bg-white rounded-xl border border-gray-100 text-center py-12">
    <div className="w-14 h-14 mx-auto mb-3 bg-gray-100 rounded-full flex items-center justify-center"><FaUsers className="text-xl text-gray-400" /></div>
...
const DetailSection = ({ icon: Icon, title, children }) => (
  <div><h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2"><Icon className="text-[#1B3766] text-sm" />{title}</h4><div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">{children}</div></div>
);

const DetailRow = ({ label, value, mono }) => (
  <p className="flex justify-between"><span className="text-gray-500">{label}:</span><span className={`font-medium text-gray-900 text-right ml-4 ${mono ? 'font-mono text-xs' : ''}`}>{value || '—'}</span></p>
);
