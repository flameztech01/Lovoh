...
const AdminDashboard = () => {
  ...
  const { data: pendingProductsData } = useGetAllProductsAdminQuery({
    isApproved: false,
  });
  const { data: ordersData, isLoading: ordersLoading } = useGetAllOrdersQuery(
    {},
  );
  const { data: sellerAppsData, isLoading: sellersLoading } =
    useGetSellerApplicationsQuery({ status: "pending" });
  ...
  useEffect(() => {
    if (reportsData?.reports) {
      const pending = reportsData.reports.filter(
        (r) => r.status === "pending",
      ).length;
      setStats((prev) => ({
        ...prev,
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: FaTag,
      color: "bg-yellow-500",
      link: "/superuser/products/pending",
    },
    {
      title: "Total Orders",
    ...
    },
    {
      ...
      icon: FaSpinner,
      color: "bg-orange-500",
      link: "/superuser/orders?status=pending",
    },
    {
      title: "Total Revenue",
    ...
    },
    {
      ...
      icon: FaStore,
      color: "bg-indigo-500",
      link: "/superuser/seller-applications",
    },
    {
      title: "Total Reports",
    ...
    },
    {
      ...
      icon: FaUsers,
      color: "bg-teal-500",
      link: "/superuser/sellers",
    },
  ];

  const isLoading =
    productsLoading || ordersLoading || sellersLoading || reportsLoading;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="p-5">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {app.sellerApplication?.businessName}
                  </p>
                </div>
                <Link
                  to={`/superuser/seller-applications/${app._id}`}
            ...
            {(!sellerAppsData?.applications ||
              sellerAppsData.applications.length === 0) && (
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Link
          to="/superuser/seller-applications"
